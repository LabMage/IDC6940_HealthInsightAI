...
The kNN algorithm calculates the euclidean distance between the unknown datapoint and the testing datapoints because it assumes similar datapoints will be in close proximity to each other and be neighbors and that data points with similar features belong to the same class. []
...
# Activate your Conda Python environment (only needed line)
use_condaenv("r-reticulate", required = TRUE)

# (Optional) Show Python info in console/log
py_config()
...
#### Train-Test Split of Resampled Dataset
...
**a. Model Performance on Imbalanced Training Data** 

The K-Nearest Neighbors (KNN) classifier was initially trained on the original, imbalanced training dataset. Evaluation was performed using the separate test set, which retains the original class distribution.
...
The model demonstrated strong accuracy (83.3%) overall, but performance was biased toward the majority class (non-diabetic). Recall for the minority class (diabetes/prediabetes) was only 0.21, indicating low sensitivity to true positive cases.


**b. Model Performance on SMOTE-Resampled Training Data**

To address the class imbalance, the training data was resampled using SMOTE (Synthetic Minority Over-sampling Technique). The KNN model was retrained on this resampled dataset and evaluated on the same test set.
...
# Build classification report
report_text_smote = f"""
KNN (SMOTE-Resampled) - Accuracy: {accuracy:.4f}
KNN (SMOTE-Resampled) - ROC-AUC Score: {roc_auc:.4f}
...
Resampling with SMOTE significantly improved the model’s ability to detect minority class instances. Recall for the diabetic class increased from 0.21 to 0.84, and the ROC-AUC score rose to 0.83, indicating better class separation.
