...
### 1.3 Statistical Summary & Variable Distribution

Understanding the statistical characteristics of key features is essential for effective preprocessing and modeling, particularly for distance-based algorithms like **k-Nearest Neighbors (kNN)**. The following visualizations illustrate central tendencies, variability, and potential issues such as **outliers** and **scale disparities** across selected ordinal and continuous variables.
...
```
Figures 3 and 4 highlight important aspects of feature distributions
that directly impact the performance of a k-Nearest Neighbors (kNN)
classifier.

#### 1.3.1 Summary Statistics Heatmap

The heatmap displays descriptive statistics (mean, standard deviation, min, quartiles, max) for the following ordinal and continues features:

- `GenHlth`, `MentHlth`, `PhysHlth`, `Age`, `Education`, `Income`, `BMI`

##### Key observations:

-   **BMI** has a much larger value range (min = 12.0, max = 98.0) and
    the highest standard deviation.
    
-   **GenHlth**, **Education**, and **Income** have much narrower ranges
    (e.g., GenHlth: 1–5, Education: 1–6).
...
```
Because features like BMI have much larger numeric values than others, they can overpower distance calculations in algorithms like kNN. This means that even if a smaller-scale feature (like Education or GenHlth) is important, it may have little effect on how the model measures similarity between data points.

To fix this, we use feature scaling - such as standardizing (z-score) or normalizing values - so that all features contribute fairly and equally to the distance calculation.


#### 1.3.2 Outliers in Distribution 

Figure 4 presents a boxplot of the same variables and shows **extreme
upper outliers** in: 

- **MentHlth** and **PhysHlth** (both up to 30) 

- **BMI**, with extreme values up to 98

Outliers can **distort distance measurements**, causing certain data
points to appear far away from all neighbors, or worse — attract other
points simply due to high magnitude. This affects classification
consistency and may reduce accuracy, especially for minority classes.
...
By applying these preprocessing steps, the distance metric in kNN
becomes more balanced and representative, improving classification
accuracy and generalization.


##### 1.3.3.Class Imbalance in Diabetes Prevalence
...
Figure 6 shows the full feature correlation matrix, allowing us to assess the strength and direction of associations across all variable types. 
...
##### 1.3.5 BMI Distribution and Density Analysis by Diabetes Status
...
**KNN 1: Model Performance on Imbalanced Training Data**

To establish a baseline, the k-Nearest Neighbors (kNN) classifier was first trained on the original, imbalanced dataset. The model used 5 neighbors, Euclidean distance, and uniform weighting—standard parameters often used in default kNN implementations.

Evaluation was performed on a separate test set that preserved the original class distribution. This setup mimics real-world deployment conditions, where class imbalance is a common challenge in health datasets.

Despite strong overall accuracy, the model struggled to correctly identify the minority class (diabetes), reflecting the limitations of kNN when trained on unbalanced data.
...
fpr_imbal, tpr_imbal, thresholds_imbal = roc_curve(y_test_imbal, y_proba_imbal)

```
Despite strong overall accuracy (83.3%), the model struggled to correctly identify the minority class (diabetes), reflecting the limitations of kNN when trained on unbalanced data.

**KNN 2: kNN with Robust Scaling on Imbalanced Data**

To test whether scaling strategy and distance weighting could improve baseline performance, the second kNN model was trained on the same imbalanced dataset, but with several key modifications.

This configuration used **k = 15, Manhattan distance (p=1)**, and **distance-based weighting** to give closer neighbors more influence. Additionally, RobustScaler was applied to scale continuous features, making the model less sensitive to outliers in health-related variables such as BMI, MentHlth, and PhysHlth.
...
# Optional: Store ROC values for plotting later
fpr_imbal, tpr_imbal, thresholds_imbal = roc_curve(y_test_imbal, y_proba_imbal)
...
**KNN 3: kNN Performance on SMOTE-Resampled Training Data**

To address class imbalance and improve the model’s sensitivity to diabetic cases, the training set was resampled using **SMOTE (Synthetic Minority Over-sampling Technique)**. This technique generates synthetic examples for the minority class, resulting in a balanced 50/50 distribution in the training data.

The kNN model was retrained using the same configuration as the baseline:
**k = 10, Euclidean distance (p=2), and uniform weighting**.
The test set was left unchanged to preserve the original class distribution, simulating real-world prediction conditions.
...
However, precision dropped, indicating an increase in false positives — a common trade-off in recall-optimized models.

**KNN 4: KNN with Feature Selection**

To evaluate whether dimensionality reduction could improve kNN performance, a Chi-Square feature selection method was applied. This statistical technique ranks features based on their dependency with the target variable (Diabetes_binary), helping identify those most informative for classification.

The top 12 features (as shown in the bar plot below) were retained for training. Continuous features such as BMI, MentHlth, and PhysHlth were scaled only if they appeared in the selected subset.
...
plt.title("Top 12 Features by Chi-Square Score")
...
```
The training set was resampled using SMOTE to ensure class balance, and the model was configured with:
**k = 15, Euclidean distance (p=2)**, and **distance-based weighting**.

The resulting model showed a notable boost in performance, achieving:
...
```
This version of kNN provided **the best trade-off between precision and recall**, suggesting that both feature selection and SMOTE contributed to more robust classification of diabetic cases.
...
The first phase of model evaluation focused exclusively on optimizing the k-Nearest Neighbors (kNN) algorithm. Four versions of kNN were trained with varying configurations, including distance metrics, weighting strategies, feature scaling techniques, and class balancing using SMOTE. Table 3 summarizes the performance of each model to identify the most effective setup.
...
As shown in Table 3, Model 4 - the version of kNN with SMOTE, feature selection, distance-based weighting, and k=15 - achieved the highest performance. It delivered the best ROC-AUC score (0.88) and Recall for class 1 (0.88), indicating strong ability to detect diabetic cases. In contrast, the baseline Model 1 struggled with recall and overall minority class performance, despite decent accuracy. This confirms the importance of both data balancing and feature reduction when using kNN on imbalanced medical datasets.
...
The Decision Tree was first trained on the original imbalanced data to observe how well it performs without resampling or feature selection.
...
```
Despite high overall accuracy (86.2%), the model struggled to detect diabetic cases, with a recall of just 15% for the minority class.

**Decision Tree – SMOTE-Resampled Dataset**

Next, the Decision Tree was retrained on a SMOTE-resampled dataset to evaluate whether balancing the classes improves recall and ROC-AUC.
...
```
SMOTE led to a more balanced performance, with recall increasing from 0.15 to 0.78 for the diabetic class, though at the cost of reduced accuracy.

**Random Forest – Imbalanced Dataset**

Random Forest was evaluated only on the imbalanced dataset. This is because RF handles class imbalance better than kNN, and using SMOTE with ensemble methods can increase overfitting due to repeated sampling of synthetic points.
...
```
Random Forest showed the best overall accuracy (86.6%) but, like the basic Decision Tree, it suffered from low recall (13%) for the diabetic class.

#### Code Implementation Comparison

To summarize model performance across algorithms, we present two final comparisons: 

**Table 4** contrasts the best-performing kNN model (kNN 4) with Decision Tree and Random Forest classifiers, while the **ROC-AUC curves** below visualize their ability to distinguish between diabetic and non-diabetic classes.
...
From Table 4, we observe that the tuned kNN model outperformed both Decision Tree variants in ROC-AUC, Recall_1, and overall balance of precision/recall. While Random Forest showed strong accuracy and specificity (Recall_0), it significantly underperformed on Recall_1, which is critical for identifying diabetic individuals. This reinforces the value of targeted tuning and data preprocessing when working with imbalanced health datasets.
...
```{python, echo=TRUE, results='hide', message=FALSE, warning=FALSE}

import matplotlib.pyplot as plt
plt.figure(figsize=(8, 6))

plt.plot(fpr_fs, tpr_fs, label=f"KNN (Feature Selection) (AUC = {roc_auc_fs:.2f})")
plt.plot(fpr_dt_sm, tpr_dt_sm, label=f"Decision Tree (SMOTE) (AUC = {roc_auc_dt_sm:.2f})")
plt.plot(fpr_dt, tpr_dt, label=f"Decision Tree (Imbalanced) (AUC = {roc_auc_dt:.2f})")
plt.plot(fpr_rf, tpr_rf, label=f"Random Forest (Imbalanced) (AUC = {roc_auc_rf:.2f})")
